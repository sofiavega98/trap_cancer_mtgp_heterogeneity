...
# Data Cleaning Script: Hispanic Ethnicity (Hispanic vs Non-Hispanic)
# 
# This script cleans and prepares SEER lymphoma incidence data for Hispanic
# ethnicity analysis, separating cases into Hispanic and Non-Hispanic categories.
#
# INPUTS:
# - SEER lymphoma data: "~/Documents/Harvard/Lymphoma_Subsets/data/hisp.csv"
# - SEER population data: "~/Documents/Harvard/Lymphoma_Subsets/data/seer_population.RData"
#
# OUTPUTS:
# - Hispanic ethnicity dataset: "~/Documents/Harvard/Lymphoma_Subsets/data/hisp_Lymphoma_1988_2003_0-29.RData"
#
# This dataset is used for:
# - GSC model fitting (1_model_run_GSC_hisp.R)
# - MTGP model fitting (2_model_run_MTGP_hisp.R)
...
# - Ethnicity groups: Hispanic, Non-Hispanic
...
# Load raw data
# SEER lymphoma incidence data with Hispanic ethnicity stratification
SEER <- read.csv(file.path(DATA_DIR, "hisp.csv"))
...
SEER_1 <- SEER %>%
  # Recode lymphoma type (all types combined for Hispanic ethnicity analysis)
  mutate(cancer_type = recode(`Site.recode.ICD.O.3.WHO.2008.Lymphoma`, "0" = "Lymphoma"),
         ...
         year_recode = case_when(`Year.of.diagnosis` == 0 ~ NA_real_,
                                 TRUE ~ `Year.of.diagnosis` + 1974) #SV Edit 10/03: change 1980 to 1974
         
  )


# Subset to be ages less than 29
SEER_final <- SEER_1 %>% select(fips, year_recode, `Age.recode.with..1.year.olds`, Count, `Origin.recode.NHIA..Hispanic..Non.Hisp.`) %>%
  ...
  select(fips, year_recode, age_recode, Count, `Origin.recode.NHIA..Hispanic..Non.Hisp.`) %>%
  group_by(fips, year_recode, `Origin.recode.NHIA..Hispanic..Non.Hisp.`) %>%
  summarise(count = sum(Count)) %>%
  rename(year = year_recode) %>%
  ungroup()

# Rename columns
colnames(SEER_final) <- c("FIPS","YEAR_DX","HISPANIC","CL_CASES") 
...
seer_data_aggregated <- seer_population_0_29 %>%
  group_by(Year, FIPS, Origin) %>%
  summarise(Population = sum(Population)) %>%
  ungroup()

## dont have census data from 1983 to 1989 so copy 1990
data_full_pop <- left_join(data_full,seer_data_aggregated, by = c("FIPS" = "FIPS", "YEAR_DX" = "Year", "HISPANIC" = "Origin"))

# Fill years prior to 1990
data_full_pop <- data_full_pop %>%
  group_by(FIPS, HISPANIC) %>%
  arrange(FIPS, HISPANIC, YEAR_DX) %>%
  fill(Population, .direction = "up") %>%
  ungroup()

# Assign column names
colnames(data_full_pop) <- c("FIPS","YEAR_DX","HISPANIC","CL_CASES","C",
                              "POP")
...
# - HISPANIC: Hispanic ethnicity group (0=Non-Hispanic, 1=Hispanic)
...
save(data_full_pop, file = file.path(DATA_DIR, paste0('hispanic_Lymphoma_',years[1],'_',years[length(years)],'_0-29.RData')))

# Print summary statistics
cat("Hispanic ethnicity data cleaning complete!\n")
cat("Final dataset dimensions:", dim(data_full_pop), "\n")
cat("Hispanic groups:", unique(data_full_pop$HISPANIC), "\n")
