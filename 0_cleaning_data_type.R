...
# Data Cleaning Script: Lymphoma Type (Hodgkin vs Non-Hodgkin)
# 
# This script cleans and prepares SEER lymphoma incidence data for lymphoma
# subtype analysis, separating cases into Hodgkin Lymphoma and Non-Hodgkin
# Lymphoma categories.
#
# INPUTS:
# - SEER lymphoma data: "data/type.csv"
# - Population data: "data/county_hisp_pop_lymphoma_0_29.RData"
#
# OUTPUTS:
# - Lymphoma type dataset: "data/type_Lymphoma_1988_2003_0-29.RData"
#
# This dataset is used for:
# - GSC model fitting (1_model_run_GSC_type.R)
# - MTGP model fitting (2_model_run_MTGP_type.R)
...
# Load raw data
# SEER lymphoma incidence data with lymphoma type stratification
SEER <- read.csv(file.path(DATA_DIR, "type.csv"))

# Load population data for rate calculations
load(file.path(DATA_DIR, "county_hisp_pop_lymphoma_0_29.RData"))

# Recode categorical variables for clarity
# Convert numeric codes to descriptive labels
SEER_1 <- SEER %>%
  # Recode lymphoma types (Hodgkin vs Non-Hodgkin)
  mutate(cancer_type = recode(`Site.recode.ICD.O.3.WHO.2008..Lymphoma`, 
                              "0" = "Hodgkin Lymphoma", 
...
SEER_final <- SEER_1 %>% select(fips, year_recode, `Age.recode.with..1.year.olds`, Count, `cancer_type`) %>%
  ...
  select(fips, year_recode, age_recode, Count, `cancer_type`) %>%
  group_by(fips, year_recode, `cancer_type`) %>%
  summarise(count = sum(Count)) %>%
  rename(year = year_recode) %>%
  ungroup()

# Rename columns
colnames(SEER_final) <- c("FIPS","YEAR_DX","TYPE","CL_CASES") 
...
data_full_pop <- left_join(data_full,county_hisp_pop, by = c("FIPS" = "fips", "YEAR_DX" = "year"))

data_full_pop <- data_full_pop %>%
  fill(c(pct_hispanic_pop, total_pop), .direction = "up") 

# Assign column names
colnames(data_full_pop) <- c("FIPS","YEAR_DX","TYPE","CL_CASES","C","pct_hispanic_pop",
                              "POP")
...
# Save final data
save(data_full_pop, file = file.path(DATA_DIR, paste0('type_Lymphoma_',years[1],'_',years[length(years)],'_0-29.RData')))

# Print summary statistics
cat("Lymphoma type data cleaning complete!\n")
cat("Final dataset dimensions:", dim(data_full_pop), "\n")
cat("Lymphoma types:", unique(data_full_pop$TYPE), "\n")
