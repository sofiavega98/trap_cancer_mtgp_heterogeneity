...
# Data Cleaning Script: Race (White, Black, Other)
# 
# This script cleans and prepares SEER lymphoma incidence data for race-based
# analysis, separating cases into three racial categories: White, Black, and Other.
#
# INPUTS:
# - SEER lymphoma data: "data/race_x3.csv"
# - SEER population data: "data/seer_population.RData"
#
# OUTPUTS:
# - Race-stratified dataset: "data/race_Lymphoma_1988_2003_0-29.RData"
#
# This dataset is used for:
# - GSC model fitting (1_model_run_GSC_race.R)
# - MTGP model fitting (2_model_run_MTGP_race.R)
...
# Load raw data
# SEER lymphoma incidence data with race stratification (3 categories)
SEER <- read.csv(file.path(DATA_DIR, "race_x3.csv"))
...
SEER_1 <- SEER %>%
  # Recode lymphoma type (all types combined for race analysis)
  mutate(cancer_type = recode(`Site.recode.ICD.O.3.WHO.2008.Lymphoma`, "0" = "Lymphoma"),
         ...
         year_recode = `Year.of.diagnosis.trt` + 1988
         
  )


# Subset to be ages less than 29
SEER_final <- SEER_1 %>% select(fips, year_recode, `Age.recode.with..1.year.olds`, Count, `Race.recode..White..Black..Other.`) %>%
  ...
  select(fips, year_recode, age_recode, Count, `Race.recode..White..Black..Other.`) %>%
  group_by(fips, year_recode, `Race.recode..White..Black..Other.`) %>%
  summarise(count = sum(Count)) %>%
  rename(year = year_recode) %>%
  ungroup()

# Rename columns
colnames(SEER_final) <- c("FIPS","YEAR_DX","RACE","CL_CASES") 

# Filter out Race = 0 (all races ) and race = 4 (race unknown)
SEER_final <- SEER_final %>% filter(RACE %in% c(1,2,3))
...
data_full_pop <- left_join(data_full,seer_data_aggregated, by = c("FIPS" = "FIPS", "YEAR_DX" = "Year", "RACE" = "Race"))

# Fill years prior to 1990
data_full_pop <- data_full_pop %>%
  group_by(FIPS, RACE) %>%
  arrange(FIPS, RACE, YEAR_DX) %>%
  fill(Population, .direction = "up") %>%
  ungroup()


# Assign column names
colnames(data_full_pop) <- c("FIPS","YEAR_DX","RACE","CL_CASES","C",
                              "POP")
...
# Remove 49017 for not having a black population
data_full_pop <- data_full_pop %>% filter(!(FIPS==49017))
...
# - RACE: Race group (1=White, 2=Black, 3=Other)
...
save(data_full_pop, file = file.path(DATA_DIR, paste0('race_x3_Lymphoma_',years[1],'_',years[length(years)],'_0-29.RData')))

# Print summary statistics
cat("Race data cleaning complete!\n")
cat("Final dataset dimensions:", dim(data_full_pop), "\n")
cat("Race groups:", unique(data_full_pop$RACE), "\n")
