...
# Model Fitting: MTGP (Multi-Task Gaussian Process) for Lymphoma Type
# 
# This script fits Multi-Task Gaussian Process (MTGP) models using Stan for
# lymphoma type analysis. It analyzes treatment effects on lymphoma incidence
# rates for Hodgkin and Non-Hodgkin lymphoma simultaneously.
#
# INPUTS:
# - Non-Hodgkin data: "~/Documents/Harvard/Lymphoma_Subsets/data/NH_Lymphoma_1988_2003_0-29.RData"
# - Hodgkin data: "~/Documents/Harvard/Lymphoma_Subsets/data/H_Lymphoma_1988_2003_0-29.RData"
# - Stan model: "~/Documents/Harvard/Lymphoma_Subsets/code/mt_gp_pois_multigroup.stan"
#
# OUTPUTS:
# - MTGP lymphoma type results: "~/Documents/Harvard/Lymphoma_Subsets/results/11132024/mtgp_fit_type_year.RData"
#
# MODEL SPECIFICATION:
# - Method: Multi-Task Gaussian Process (MTGP) via Stan
# - Outcome: Lymphoma type-stratified case counts (Poisson)
# - Treatment: Binary indicator for CA/CT counties after 1995
# - Population: Lymphoma type-specific population denominators
...
# - Lymphoma types: Hodgkin (H) and Non-Hodgkin (NH)
# - Geographic scope: SEER registry counties (filtered for non-zero cases)
################################################################################
...
# Load lymphoma type-stratified data
load(file.path(DATA_DIR, "H_Lymphoma_1988_2003_0-29.RData"))
data_full_pop_H <- data_full_pop_H %>% select("FIPS", "YEAR_DX", "CL_CASES", "C", "POP")
data_full_pop_H$Cohort <- "H"

load(file.path(DATA_DIR, "NH_Lymphoma_1988_2003_0-29.RData"))
data_full_pop_NH <- data_full_pop_NH %>% select("FIPS", "YEAR_DX", "CL_CASES", "C", "POP")
data_full_pop_NH$Cohort <- "NH"

# Combine lymphoma type groups into one dataframe for MTGP analysis
data_full <- rbind(data_full_pop_H, data_full_pop_NH)
# 181 unique counties

# Use example cohort to get treatment information
ex_cohort <- data_full %>% filter(Cohort == "H")
...
num_outcomes = length(unique(data_full$Cohort)) # Number of outcomes (lymphoma type groups)
...
# Save MTGP results
save(fit, file = file.path(RESULTS_DIR, date, "mtgp_fit_type_year.RData"))

# Print completion message
cat("MTGP model for lymphoma type groups completed!\n")
cat("Results saved in:", file.path(RESULTS_DIR, date, "mtgp_fit_type_year.RData"), "\n")
