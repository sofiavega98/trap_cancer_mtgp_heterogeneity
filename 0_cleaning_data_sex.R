...
# Data Cleaning Script: Sex (Male vs Female)
# 
# This script cleans and prepares SEER lymphoma incidence data for sex-based
# analysis, separating cases into male and female categories.
#
# INPUTS:
# - SEER lymphoma data: "data/sex.csv"
# - SEER population data: "data/seer_population.RData"
#
# OUTPUTS:
# - Sex-stratified dataset: "data/sex_Lymphoma_1988_2003_0-29.RData"
#
# This dataset is used for:
# - GSC model fitting (1_model_run_GSC_sex.R)
# - MTGP model fitting (2_model_run_MTGP_sex.R)
...
# Load raw data
# SEER lymphoma incidence data with sex stratification
SEER <- read.csv(file.path(DATA_DIR, "sex.csv"))
...
SEER_1 <- SEER %>%
  # Recode lymphoma type (all types combined for sex analysis)
...
SEER_final <- SEER_1 %>% select(fips, year_recode, `Age.recode.with..1.year.olds`, Count, `Sex`) %>%
...
data_full_pop <- left_join(data_full,seer_data_aggregated, by = c("FIPS" = "FIPS", "YEAR_DX" = "Year", "SEX" = "Sex"))

# Remove Sex ==0
data_full_pop <- data_full_pop %>% filter(SEX != 0)
...
# Assign column names
colnames(data_full_pop) <- c("FIPS","YEAR_DX","SEX","CL_CASES","C",
                              "POP")
...
save(data_full_pop, file = file.path(DATA_DIR, paste0('sex_Lymphoma_',years[1],'_',years[length(years)],'_0-29.RData')))

# Print summary statistics
cat("Sex data cleaning complete!\n")
cat("Final dataset dimensions:", dim(data_full_pop), "\n")
cat("Sex groups:", unique(data_full_pop$SEX), "\n")
