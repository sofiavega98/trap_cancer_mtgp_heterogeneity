################################################################################
# Data Cleaning: Total Population
# 
# This script cleans and prepares the SEER lymphoma data for the total population
# analysis. It filters the data to the study period, creates treatment indicators,
# and merges with population data for rate calculations.
#
# INPUTS:
# - SEER lymphoma data: "data/lymphoma_full.csv"
# - SEER population data: "data/seer_population.RData"
#
# OUTPUTS:
# - Cleaned dataset: "data/total_Lymphoma_1988_2003_0-29.RData"
#
# STUDY DESIGN:
...
# Create exploratory plot of age distribution
# This helps verify the age structure of the data
ggplot(SEER_1, aes(x = Age.recode.with..1.year.olds, y = Count)) +
  geom_bar(stat = "identity") +
  labs(title = "Counts by Age Category", x = "Age Category", y = "Count") +
  theme_minimal()

# Filter data to include only ages 0-29 and valid FIPS codes
df <- SEER_1 %>% filter(`Age.recode.with..1.year.olds` <= 6,
                  !is.na(fips),
                  !is.na(year_recode))

# Create final cleaned dataset
# This step aggregates cases by county and year, combining all age groups 0-29
SEER_final <- SEER_1 %>%
  select(fips, year_recode, `Age.recode.with..1.year.olds`, Count) %>%
  filter(`Age.recode.with..1.year.olds` <= 6,  # Ages 0-29 (codes 0-6)
         !is.na(fips),
         !is.na(year_recode)) %>%
  ...
  summarise(count = sum(Count)) %>%  # Sum cases across all age groups
...
# Filter to study period (1988-2003)
...
# Hawaii counties changed FIPS codes in 2000, so we need to combine pre- and post-2000 data
HI_pre2000 <- c(15911, 15912, 15913, 15914, 15915)  # Pre-2000 FIPS codes
HI_post2000 <- c(15001, 15003, 15005, 15007, 15009) # Post-2000 FIPS codes
...
# Remove old Hawaii counties (pre-2000 codes)
...
# This identifies which counties and years received the treatment
data_full <- data_full %>% mutate(
  # Specify CT as treated after 1995
  C = case_when(
    str_starts(FIPS, "09") & YEAR_DX >= treated_year ~ 1,
    TRUE ~ 0),
  # Specify CA as treated after 1995
  C = case_when(
...
# Subset population data to study period
seer_population_0_29 <- seer_population %>% filter(Year %in% years)

# Remove demographic variables not needed for overall analysis
...
seer_population_0_29$Sex <- NULL

# Aggregate population data by year and county
seer_data_modified <- seer_data_aggregated %>%
  group_by(Year, FIPS) %>%
  summarise(Population = sum(Population, na.rm = TRUE)) %>%
  ungroup()

# Merge case data with population data
data_full_pop <- left_join(data_full,seer_data_modified, by = c("FIPS" = "FIPS", "YEAR_DX" = "Year"))

# Handle missing population data for years prior to 1990
# Fill missing values using forward fill (copy 1990 values to earlier years)
data_full_pop <- data_full_pop %>%
  group_by(FIPS) %>%
  arrange(FIPS,  YEAR_DX) %>%
  fill(Population, .direction = "up") %>%
  ungroup()

# Assign final column names
colnames(data_full_pop) <- c("FIPS","YEAR_DX","CL_CASES","C","POP")

# Remove FIPS 15005 (incorporated into 15009 in Hawaii)
data_full_pop <- data_full_pop %>% filter(!(FIPS==15005))

# Remove unknown counties from the dataset
data_full_pop <- data_full_pop %>% filter(! (FIPS %in% c("09999", "15999", "35999", "49999")) )

# Optional: Remove counties with zero cases across all years
...
#data_full_pop_noZero <- data_full_pop %>%
#  filter(! FIPS %in% zero_case_FIPS)

# Save final cleaned dataset
# This file will be used by subsequent analysis scripts
save(data_full_pop, file = file.path(DATA_DIR, paste0('total_Lymphoma_',years[1],'_',years[length(years)],'_0-29.RData')))

cat("✓ Overall population data cleaning completed\n")
cat("  - Study period:", min(years), "-", max(years), "\n")
cat("  - Treatment year:", treated_year, "\n")
cat("  - Age range: 0-29 years\n")
cat("  - Counties included:", length(unique(data_full_pop$FIPS)), "\n")
cat("  - Total observations:", nrow(data_full_pop), "\n")
cat("  - Treated observations:", sum(data_full_pop$C), "\n")
