################################################################################
# Data Cleaning: Age-Stratified Analysis
# 
# This script cleans and prepares the SEER lymphoma data for age-stratified
...
#
# OUTPUTS:
# - Age-stratified dataset: "data/age_Lymphoma_1988_2003_0-29.RData"
#
# STUDY DESIGN:
# - Study period: 1988-2003 (16 years)
# - Treatment year: 1995 (California and Connecticut implement treatment)
# - Age range: 0-29 years (stratified into 0-19 and 20-29)
...
cat("Cleaning data for age-stratified analysis...\n")
...
SEER_1 <- SEER %>%
  # Recode lymphoma type (all types combined for age analysis)
...
ggplot(df, aes(x = Age.recode.with..1.year.olds, y = Count)) +
  geom_bar(stat = "identity") +
  labs(title = "Counts by Age Category", x = "Age Category", y = "Count") +
  theme_minimal()

# Find median age category
df <- df[order(df$`Age.recode.with..1.year.olds`), ]  # Ensure the data is ordered by age (if not already)
...
# Find the median category, where cumulative count >= 50% of total count
median_category <- df$`Age.recode.with..1.year.olds`[which(df$cumulative_count >= total_count / 2)[1]]

median_category

# Subset to be ages less than 29 and within each cohort of interest
SEER_final <- SEER_1 %>%
  ...
  mutate(age_recode = recode(`Age.recode.with..1.year.olds`,
                             ...
                             "6"="25-29 years"),
         cohort = case_when(
           `Age.recode.with..1.year.olds` <= 4 ~ "Younger Age (0-4)",
           `Age.recode.with..1.year.olds` >= 5 ~ "Older Age (5-6)"
         )) %>%
  select(fips, year_recode, age_recode, cohort, Count) %>%
  group_by(fips, year_recode, cohort) %>%
...
# Rename columns
colnames(SEER_final) <- c("FIPS","YEAR_DX","AGE","CL_CASES") 
...
seer_data_modified <- seer_data_aggregated %>%
  mutate(Age_Group = case_when(
    Age <= 4 ~ "Younger Age (0-4)",
    Age >= 5 ~ "Older Age (5-6)",
    TRUE ~ NA_character_  # This catches any unexpected values
  )) %>%
  group_by(Year, FIPS, Age_Group) %>%
  summarise(Population = sum(Population, na.rm = TRUE)) %>%
  ungroup()

## dont have census data from 1983 to 1989 so copy 1990
data_full_pop <- left_join(data_full,seer_data_modified, by = c("FIPS" = "FIPS", "YEAR_DX" = "Year", "AGE" = "Age_Group"))

# Fill years prior to 1990
data_full_pop <- data_full_pop %>%
  group_by(FIPS, AGE) %>%
  arrange(FIPS, AGE, YEAR_DX) %>%
  fill(Population, .direction = "up") %>%
  ungroup()

# Assign final column names for consistency
colnames(data_full_pop) <- c("FIPS","YEAR_DX","AGE","CL_CASES","C",
                              "POP")
...
# Optional: Remove counties with all zero cases (commented out)
...
# - YEAR_DX: Year of diagnosis (1988-2003)
# - AGE: Age group ("Younger Age (0-4)" or "Older Age (5-6)")
# - CL_CASES: Count of lymphoma cases
# - C: Treatment indicator (1 for CA/CT counties after 1995, 0 otherwise)
# - POP: Population counts for rate calculations
save(data_full_pop, file = file.path(DATA_DIR, paste0('age_Lymphoma_',years[1],'_',years[length(years)],'_0-29.RData')))

# Print summary statistics
cat("Age data cleaning complete!\n")
cat("Final dataset dimensions:", dim(data_full_pop), "\n")
cat("Age groups:", unique(data_full_pop$AGE), "\n")
cat("Years:", range(data_full_pop$YEAR_DX), "\n")
cat("Total counties:", length(unique(data_full_pop$FIPS)), "\n")
cat("Total cases:", sum(data_full_pop$CL_CASES), "\n")
