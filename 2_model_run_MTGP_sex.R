...
# Model Fitting: MTGP (Multi-Task Gaussian Process) for Sex
# 
# This script fits Multi-Task Gaussian Process (MTGP) models using Stan for
# sex-based analysis. It analyzes treatment effects on lymphoma incidence
# rates for males and females simultaneously.
#
# INPUTS:
# - Male data: "~/Documents/Harvard/Lymphoma_Subsets/data/male_Lymphoma_1988_2003_0-29.RData"
# - Female data: "~/Documents/Harvard/Lymphoma_Subsets/data/female_Lymphoma_1988_2003_0-29.RData"
# - Stan model: "~/Documents/Harvard/Lymphoma_Subsets/code/mt_gp_pois_multigroup.stan"
#
# OUTPUTS:
# - MTGP sex results: "~/Documents/Harvard/Lymphoma_Subsets/results/11132024/mtgp_fit_sex_year.RData"
#
# MODEL SPECIFICATION:
# - Method: Multi-Task Gaussian Process (MTGP) via Stan
# - Outcome: Sex-stratified lymphoma case counts (Poisson)
# - Treatment: Binary indicator for CA/CT counties after 1995
# - Population: Sex-specific population denominators
...
data_full_pop_male <- data_full_pop_male %>% select("FIPS", "YEAR_DX", "CL_CASES", "C", "POP")
data_full_pop_male$Cohort <- "male"

load(file.path(DATA_DIR, "female_Lymphoma_1988_2003_0-29.RData"))
data_full_pop_female <- data_full_pop_female %>% select("FIPS", "YEAR_DX", "CL_CASES", "C", "POP")
data_full_pop_female$Cohort <- "female"

# Combine sex groups into one dataframe for MTGP analysis
data_full <- rbind(data_full_pop_male, data_full_pop_female)
# 181 unique counties

# Use example cohort to get treatment information
ex_cohort <- data_full %>% filter(Cohort == "male")
...
num_outcomes = length(unique(data_full$Cohort)) # Number of outcomes (sex groups)
...
# Save MTGP results
save(fit, file = file.path(RESULTS_DIR, date, "mtgp_fit_sex_year.RData"))

# Print completion message
cat("MTGP model for sex groups completed!\n")
cat("Results saved in:", file.path(RESULTS_DIR, date, "mtgp_fit_sex_year.RData"), "\n") 
