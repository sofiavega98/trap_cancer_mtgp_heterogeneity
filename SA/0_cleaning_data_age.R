...
# Sensitivity Analysis: Data Cleaning Script - Age Groups (0-19 vs 20-29)
# 
# This script cleans and prepares SEER lymphoma incidence data for age group
# sensitivity analysis, using an extended study period (1983-2003) and earlier
# treatment year (1990) to test robustness of main findings.
#
# INPUTS:
# - SEER lymphoma data: "~/Documents/Harvard/Lymphoma_Subsets/data/lymphoma_full_1983_2003.csv"
# - SEER population data: "~/Documents/Harvard/Lymphoma_Subsets/data/seer_population.RData"
#
# OUTPUTS:
# - Age-stratified sensitivity dataset: "~/Documents/Harvard/Lymphoma_Subsets/data/age_Lymphoma_1983_2003_0-29.RData"
#
# This dataset is used for:
# - MC model fitting (1_model_run_MC_age.R)
# - MTGP model fitting (2_model_run_MTGP_age.R)
# - Results generation (3_export_results.R)
#
# SENSITIVITY ANALYSIS DESIGN:
# - Extended study period: 1983-2003 (21 years vs 16 years in main analysis)
# - Earlier treatment year: 1990 (vs 1995 in main analysis)
...
## Feature: Age Groups (0-19 vs 20-29) - Sensitivity Analysis
...
# Load raw data
# SEER lymphoma incidence data with extended time period for sensitivity analysis
SEER <- read.csv(file.path(DATA_DIR, "lymphoma_full_1983_2003.csv"))

# Load SEER population data for rate calculations
...
SEER_1 <- SEER %>%
  ...
  mutate(cancer_type = recode(`Lymphoma`, "0" = "Lymphoma"),
         ...
         year_recode = case_when(
                                 TRUE ~ `Year.of.diagnosis.subset` + 1983)
  )

# Plot age vs. counts
ggplot(SEER_1, aes(x = Age.subset, y = Count)) +
  geom_bar(stat = "identity") +
  labs(title = "Counts by Age Category", x = "Age Category", y = "Count") +
  theme_minimal()

df <- SEER_1 %>% filter(`Age.subset` <= 6,
                  !is.na(fips),
                  !is.na(year_recode))
...
df <- df[order(df$`Age.subset`), ]  # Ensure the data is ordered by age (if not already)
...
median_category <- df$`Age.subset`[which(df$cumulative_count >= total_count / 2)[1]]

median_category

# Subset to be ages less than 29 and within each cohort of interest
SEER_final <- SEER_1 %>%
  select(fips, year_recode, `Age.subset`, Count) %>%
  filter(`Age.subset` <= 6,
         !is.na(fips),
         !is.na(year_recode)) %>%
  mutate(age_recode = recode(`Age.subset`,
                             ...
                             "6"="25-29 years"),
         cohort = case_when(
           `Age.subset` <= 4 ~ "Younger Age (0-4)",
           `Age.subset` >= 5 ~ "Older Age (5-6)"
         )) %>%
...
data_full_hisp <- left_join(data_full,seer_data_modified, by = c("FIPS" = "FIPS", "YEAR_DX" = "Year", "AGE" = "Age_Group"))
...
# Assign column names
colnames(data_full_hisp) <- c("FIPS","YEAR_DX","AGE","CL_CASES","C",
                              "POP")
...
# - YEAR_DX: Year of diagnosis (1983-2003)
# - AGE: Age group ("Younger Age (0-4)" or "Older Age (5-6)")
# - CL_CASES: Count of lymphoma cases
# - C: Treatment indicator (1 for CA/CT counties after 1990, 0 otherwise)
# - POP: Population counts for rate calculations
save(data_full_hisp_noZero, file = file.path(DATA_DIR, paste0('age_Lymphoma_',years[1],'_',years[length(years)],'_0-29.RData')))

# Print summary statistics
cat("Sensitivity analysis age data cleaning completed!\n")
cat("Final dataset dimensions:", dim(data_full_hisp_noZero), "\n")
cat("Age groups:", unique(data_full_hisp_noZero$AGE), "\n")
