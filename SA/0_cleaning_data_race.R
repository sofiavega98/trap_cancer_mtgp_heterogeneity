...
# Load cleaned data
SEER <- read.csv(file.path(DATA_DIR, "race_1983_2003.csv"))
...
SEER_final <- SEER_1 %>% select(fips, year_recode, `Age.subset`, Count, `Race.recode..White..Black..Other.`) %>%
  filter(`Age.subset` <= 6,
         !is.na(fips),
         !is.na(year_recode)) %>%
...
data_full_hisp <- left_join(data_full,seer_data_aggregated, by = c("FIPS" = "FIPS", "YEAR_DX" = "Year", "RACE" = "Race"))
...
# Assign column names
colnames(data_full_hisp) <- c("FIPS","YEAR_DX","RACE","CL_CASES","C",
                              "POP")
...
# Save final data
save(data_full_hisp_noZero, file = file.path(DATA_DIR, paste0('race_x3_Lymphoma_',years[1],'_',years[length(years)],'_0-29.RData')))

# Print summary statistics
cat("Sensitivity analysis race data cleaning completed!\n")
cat("Final dataset dimensions:", dim(data_full_hisp_noZero), "\n")
cat("Race groups:", unique(data_full_hisp_noZero$RACE), "\n")
...
# Additional race analysis with 4 groups (including Hispanic as separate race)
# This creates a 4-category race variable for additional sensitivity analysis
data_full_hisp_noZero$RACE_4 <- ifelse(data_full_hisp_noZero$RACE == 4, 4, data_full_hisp_noZero$RACE)

# Save 4-category race dataset
save(data_full_hisp_noZero, file = file.path(DATA_DIR, paste0('race_x4_Lymphoma_',years[1],'_',years[length(years)],'_0-29.RData')))

cat("4-category race dataset also saved for additional sensitivity analysis\n")
