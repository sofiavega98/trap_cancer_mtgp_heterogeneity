...
# Load cleaned data
SEER <- read.csv(file.path(DATA_DIR, "type_1983_2003.csv"))
load(file.path(DATA_DIR, "county_hisp_pop_lymphoma_0_29.RData"))

# Recode data
SEER_1 <- SEER %>%
  mutate(cancer_type = recode(`Lymphoma.Type`, "0" = "Hodgkin Lymphoma", "1" = "Non-Hodgkin Lymphoma"),
...
SEER_final <- SEER_1 %>% select(fips, year_recode, `Age.subset`, Count, `cancer_type`) %>%
  filter(`Age.subset` <= 6,
         !is.na(fips),
         !is.na(year_recode)) %>%
...
# Assign column names
colnames(data_full_hisp) <- c("FIPS","YEAR_DX","TYPE","CL_CASES","C","pct_hispanic_pop",
                              "POP")
...
# - TYPE: Lymphoma type group ("Hodgkin Lymphoma" or "Non-Hodgkin Lymphoma")
...
save(data_full_hisp_noZero, file = file.path(DATA_DIR, paste0('type_Lymphoma_',years[1],'_',years[length(years)],'_0-29.RData')))

# Print summary statistics
cat("Sensitivity analysis lymphoma type data cleaning completed!\n")
cat("Final dataset dimensions:", dim(data_full_hisp_noZero), "\n")
cat("Lymphoma types:", unique(data_full_hisp_noZero$TYPE), "\n")
