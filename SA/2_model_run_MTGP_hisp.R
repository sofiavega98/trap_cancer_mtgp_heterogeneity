...
# Load data
load(file.path(DATA_DIR, "SA/nonHisp_Lymphoma_1983_2003_0-29.RData"))
data_full_hisp_nonHisp <- data_full_hisp_nonHisp %>% select("FIPS", "YEAR_DX", "CL_CASES", "C", "POP")
data_full_hisp_nonHisp$Cohort <- "nonHisp"

load(file.path(DATA_DIR, "SA/Hisp_Lymphoma_1983_2003_0-29.RData"))
data_full_hisp_Hisp <- data_full_hisp_Hisp %>% select("FIPS", "YEAR_DX", "CL_CASES", "C", "POP")
data_full_hisp_Hisp$Cohort <- "Hisp"

# Combine into one dataframe
data_full <- rbind(data_full_hisp_nonHisp , data_full_hisp_Hisp)
#181 unique counties
...
# Save MTGP results
save(fit, file = file.path(RESULTS_DIR, date, "SA/mtgp_fit_hisp_year.RData"))

# Print completion message
cat("MTGP model for Hispanic ethnicity groups (sensitivity analysis) completed!\n")
cat("Results saved in:", file.path(RESULTS_DIR, date, "SA/mtgp_fit_hisp_year.RData"), "\n")
