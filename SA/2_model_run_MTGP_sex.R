...
# Load data
load(file.path(DATA_DIR, "SA/male_Lymphoma_1983_2003_0-29.RData"))
data_full_hisp_male <- data_full_hisp_male %>% select("FIPS", "YEAR_DX", "CL_CASES", "C", "POP")
data_full_hisp_male$Cohort <- "male"

load(file.path(DATA_DIR, "SA/female_Lymphoma_1983_2003_0-29.RData"))
data_full_hisp_female <- data_full_hisp_female %>% select("FIPS", "YEAR_DX", "CL_CASES", "C", "POP")
data_full_hisp_female$Cohort <- "female"

# Combine into one dataframe
data_full <- rbind(data_full_hisp_male, data_full_hisp_female)
#181 unique counties
...
# Save MTGP results
save(fit, file = file.path(RESULTS_DIR, date, "SA/mtgp_fit_sex_year.RData"))

# Print completion message
cat("MTGP model for sex groups (sensitivity analysis) completed!\n")
cat("Results saved in:", file.path(RESULTS_DIR, date, "SA/mtgp_fit_sex_year.RData"), "\n")
