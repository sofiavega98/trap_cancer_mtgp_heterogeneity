...
# Load cleaned data
SEER <- read.csv(file.path(DATA_DIR, "hispanic_1983_2003.csv"))
...
SEER_final <- SEER_1 %>% select(fips, year_recode, `Age.subset`, Count, `Origin.recode.NHIA..Hispanic..Non.Hisp.`) %>%
  filter(`Age.subset` <= 6,
         !is.na(fips),
         !is.na(year_recode)) %>%
...
data_full_hisp <- left_join(data_full,seer_data_aggregated, by = c("FIPS" = "FIPS", "YEAR_DX" = "Year", "HISPANIC" = "Origin"))
...
# Assign column names
colnames(data_full_hisp) <- c("FIPS","YEAR_DX","HISPANIC","CL_CASES","C",
                              "POP")
...
save(data_full_hisp_noZero, file = file.path(DATA_DIR, paste0('hispanic_Lymphoma_',years[1],'_',years[length(years)],'_0-29.RData')))

# Print summary statistics
cat("Sensitivity analysis Hispanic ethnicity data cleaning completed!\n")
cat("Final dataset dimensions:", dim(data_full_hisp_noZero), "\n")
cat("Hispanic groups:", unique(data_full_hisp_noZero$HISPANIC), "\n")
